...
function load(id, force, cb) {
  ...
  if (!force) {
    q.push("AND NOT EXISTS (SELECT FROM entity WHERE entity_id = e.entity_id AND entity_removed IS NOT NULL)");
  }
...
function loadVersion(versionId, force, cb) {
  ...
  const q = [
    "SELECT entity_id, created, doc, version_id, correlation_id",
    "FROM entity_version",
    "WHERE version_id = $1"
  ];

  if (!force) {
    q.push("AND NOT EXISTS (SELECT FROM entity WHERE entity_id = entity_id AND entity_removed IS NOT NULL)");
  }
