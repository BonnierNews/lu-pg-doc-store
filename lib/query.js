...
function upsertWithForce(entity, force, done) {
  ...
  const queryStr = [
    ...
    (force ? "VALUES(uuid_generate_v4(), $1::text, $3::jsonb, $4, $5::timestamptz)" : "SELECT uuid_generate_v4(), $1::text, $3::jsonb, $4, $5::timestamptz"),
    ...
    "SELECT $1::text, $2::text, created::timestamptz, version_id::text FROM ev",
    "ON CONFLICT ON CONSTRAINT entity_pkey",
    "DO UPDATE SET latest_version_id = (SELECT version_id::text FROM ev)",
    "RETURNING latest_version_id as version_id"
  ].join(" ");

  pgClient.query(queryStr, [entity.id, entity.type, entity, entity.meta.correlationId, new Date().toISOString()], (err, res) => {
    if (err) return done(err);
    const versionId = res.rows[0] && res.rows[0].version_id;
    return done(null, {
      entityId: entity.id,
      wasInsert: res.oid > 0,
