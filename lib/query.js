...
function queryByRelationship(options, cb) {
  const { id, system, relationType, entityType, errorOnNotFound } = options;
  const relationships = {
    id,
    type: relationType,
  };
...
function loadByExternalId(options, cb) {
  ...
  pgClient.query(
    q.join(" "),
    [options.entityType, `{"${options.systemName}": {"${options.externalIdType}": ${options.id}}}`],
    (err, res) => {
      if (err) return cb(err);
      let doc = null;
...
function remove(id, correlationId, cb) {
  ...
  load(id, (err, doc) => {
    ...
    return upsert(emptyDoc, (upsertErr) => {
      ...
      const q = ["UPDATE entity", "SET entity_removed = now() at time zone 'utc'", "WHERE entity_id = $1"];
...
function maybeUnRemove(id, cb) {
  const q = ["UPDATE entity", "SET entity_removed = null", "WHERE entity_id = $1"];
...
function listVersions(entityId, force, cb) {
  ...
  const q = [
    "SELECT ev.version_id, ev.created, ev.correlation_id, e.latest_version_id",
    "FROM entity_version as ev, entity as e",
    "WHERE ev.entity_id = $1",
    "AND ev.entity_id = e.entity_id",
  ];

  if (!force) {
    q.push("AND entity_removed IS NULL");
  }
  ...
  pgClient.query(q.join(" "), [entityId], (err, result) => {
    ...
    const output = result.rows.map((row) => ({
      ...
      status: row.version_id === row.latest_version_id ? "current" : "previously_published",
    }));
    return cb(null, output);
  });
}
...
function upsertWithForce(entity, force, done) {
  ...
  const queryStr = [
    ...
    force
      ? "VALUES(uuid_generate_v4(), $1::varchar(36), $3::jsonb, $4, $5::timestamptz)"
      : "SELECT uuid_generate_v4(), $1::varchar(36), $3::jsonb, $4, $5::timestamptz",
  ...
  pgClient.query(
    queryStr,
    [entity.id, entity.type, entity, entity.meta.correlationId, new Date().toISOString()],
    (err, res) => {
...
function removeEntityVersions(id, lastVersionId, cb) {
  pgClient.query(
    ["DELETE FROM entity_version", "WHERE entity_id = $1", "AND version_id != $2"].join(" "),
    [id, lastVersionId],
    (err) => {
      if (err) return cb(err);
      return cb(null);
